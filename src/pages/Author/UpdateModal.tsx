...
const UpdateModal: React.FC<AppProps> = ({
  ...
  const { data: author, isLoading } = useFetch<Category>(`/author/${id}`);
  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      setIsToggled(author?.status);
    }
  }, [author, setValue, isLoading, setIsToggled]);
    ...
    <div className="bg-gray-50 h-auto sm:h-84 lg:h-auto w-full rounded-md p-10 overflow-y-auto">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="font-sans font-bold text-lg">Alterar autor</h1>
          <button
            type="button"
        ...
        <form className="space-y-1" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="display flex items-center justify-end pt-5">
            <Button title="Alterar autor" type="submit" />
          </div>
        </form>
      </div>
    </div>
